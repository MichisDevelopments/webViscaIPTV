...
# Based on visca_over_ip module but modified for TCP
...
class Camera:
    ...
    def __init__(self, ip: str, port=52381, mode='udp'):
        ...
        if mode == 'udp':
            ...
            self._sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)  # for TCP stuff
            self._sock.settimeout(0.1)
    ...
    def _send_command(self, command_hex: str, query=False) -> Optional[bytes]:
        ...
        # M.E.20220602: to allow cancel command which is '81 21 FF'
        ...
        for retry_num in range(self.num_retries):
            ...
            if self._sock.type == socket.SocketKind.SOCK_DGRAM:
                self._sock.sendto(message, self._location)
            elif self._sock.type == socket.SocketKind.SOCK_STREAM:
